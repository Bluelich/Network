...
@property (nonatomic, copy, nullable) NSData *     hostAddress;
@property (nonatomic, assign ) uint16_t     nextSequenceNumber;

// private properties

/*! True if nextSequenceNumber has wrapped from 65535 to 0.
 */
@property (nonatomic, assign)BOOL  nextSequenceNumberHasWrapped;

/*! A host object for name-to-address resolution.
 * __attribute__ ((NSObject)) 用来修饰属性，表明Core Fundation类型的属性应该按照Objective-C的对象来进行内存管理
 */
@property (nonatomic, strong, nullable) __attribute__ ((NSObject)) CFHostRef host;

/*! A socket object for ICMP send and receive.
 * __attribute__ ((NSObject)) 用来修饰属性，表明Core Fundation类型的属性应该按照Objective-C的对象来进行内存管理
 */
@property (nonatomic, strong, nullable) __attribute__ ((NSObject)) CFSocketRef socket ;
...
- (addr_sin_family)hostAddressFamily {
    addr_sin_family result;
    
    result = addr_sin_family_AF_UNSPEC;
...
- (void)sendPingWithData:(NSData *)data {
    ...
    switch (self.hostAddressFamily) {
        case addr_sin_family_AF_INET: {
            packet = [self pingPacketWithType:ICMPTypeEchoRequest_v4 payload:payload requiresChecksum:YES];
        } break;
        case addr_sin_family_AF_INET6: {
            packet = [self pingPacketWithType:ICMPTypeEchoRequest_v6 payload:payload requiresChecksum:NO];
        } break;
        default: {
            assert(NO);
        } break;
...
+ (NSUInteger)icmpHeaderOffsetInIPv4Packet:(NSData *)packet {
    ...
    result = NSNotFound;
    if (packet.length >= (sizeof(struct IPv4Header) + sizeof(struct ICMPHeader))) {
        ipPtr = (const struct IPv4Header *) packet.bytes;
        if ( ((ipPtr->versionAndHeaderLength & 0xF0) == 0x40) &&            // IPv4
            ( ipPtr->protocol == IPPROTO_ICMP ) ) {
            ipHeaderLength = (ipPtr->versionAndHeaderLength & 0x0F) * sizeof(uint32_t);
            if (packet.length >= (ipHeaderLength + sizeof(struct ICMPHeader))) {
...
- (BOOL)validatePing4ResponsePacket:(NSMutableData *)packet sequenceNumber:(uint16_t *)sequenceNumberPtr {
    ...
    if (icmpHeaderOffset != NSNotFound) {
        ...
        if (receivedChecksum == calculatedChecksum) {
            if ( (icmpPtr->type == ICMPTypeEchoReply_v4) && (icmpPtr->code == 0) ) {
...
- (BOOL)validatePing6ResponsePacket:(NSMutableData *)packet sequenceNumber:(uint16_t *)sequenceNumberPtr {
    ...
    if (packet.length >= sizeof(*icmpPtr)) {
        ...
        if ( (icmpPtr->type == ICMPTypeEchoReply_v6) && (icmpPtr->code == 0) ) {
            if ( OSSwapBigToHostInt16(icmpPtr->identifier) == self.identifier ) {
                uint16_t    sequenceNumber;
...
- (void)startWithHostAddress {
    ...
    switch (self.hostAddressFamily) {
        case addr_sin_family_AF_INET: {
            fd = socket(addr_sin_family_AF_INET, SOCK_DGRAM, IPPROTO_ICMP);
            if (fd < 0) {
                err = errno;
            }
        } break;
        case addr_sin_family_AF_INET6: {
            fd = socket(addr_sin_family_AF_INET6, SOCK_DGRAM, IPPROTO_ICMPV6);
            if (fd < 0) {
                err = errno;
            }
        } break;
        ...
        for (NSData * address in addresses) {
            ...
            if ( address.length >= sizeof(struct sockaddr) ) {
                switch (addrPtr->sa_family) {
                    ...
                    case addr_sin_family_AF_INET6: {
                        if (self.addressStyle != PingAddressStyleICMPv4) {
                            self.hostAddress = address;
                            resolved = true;
                        }
                    } break;
