 ...
 */
@property (nonatomic, weak, nullable) id<PingDelegate> delegate;

/*! Controls the IP address version used by the object.
 *  \details You should set this value before starting the object.
 */
@property (nonatomic, assign) PingAddressStyle addressStyle;
 ...
 */
@property (nonatomic, assign, readonly) addr_sin_family hostAddressFamily;
 ...
 */
- (void)simplePing:(Ping *)pinger didStartWithAddress:(NSData *)address;
 ...
 */
- (void)simplePing:(Ping *)pinger didFailWithError:(NSError *)error;
 ...
 */
- (void)simplePing:(Ping *)pinger didSendPacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber;
 ...
 *  \param error Describes the failure.
 */
- (void)simplePing:(Ping *)pinger didFailToSendPacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber error:(NSError *)error;
 ...
 *  \param sequenceNumber The ICMP sequence number of that packet.
 */
- (void)simplePing:(Ping *)pinger didReceivePingResponsePacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber;
 ...
 *      follows that in the ICMP message but does not include any IP-level headers.
 */
- (void)simplePing:(Ping *)pinger didReceiveUnexpectedPacket:(NSData *)packet;
